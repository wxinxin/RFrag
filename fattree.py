...
def traffic_generation(net, topo, flows_peers):
	...
	# First packets.
	for client in (set(topo.HostList) - serversList):   # topo.HostList
		client = net.get(client)
		for server in (set(topo.HostList) - serversList):   # for server in topo.HostList
			server = net.get(server)
			if client != server:
				client.cmd("ping -c 1 -n -q %s >> %s/%s &" % (server.IP(), args.output_dir, 'first_packets.txt'))

	# Successive packets.
	for client in (set(topo.HostList) - serversList):
		client = net.get(client)
		for server in (set(topo.HostList) - serversList):   # for server in topo.HostList
			server = net.get(server)
			if client != server:
...
def run_experiment(pod, density, ip="192.168.11.132", port=6653, bw_c2a=100, bw_a2e=100, bw_e2h=100):
	"""
	...
	fanout = args.k
	Controller_Ryu = Popen("ryu-manager --observe-links ./EFattree/EFattree.py --k_paths=%d --weight=hop --fanout=%d" % (k_paths, fanout), shell=True, preexec_fn=os.setsid)
