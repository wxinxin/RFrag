...
class NetworkMonitor(app_manager.RyuApp):
	...
	def _GlobalFirstFit1(self, flow):
		'''
		...
		ftotolp = 0.0
		totolp = 0.0
		fslice = random.random()
        
		for path in paths:
		    
		    
			if (self.get_min_bw_of_links(graph, path, setting.MAX_CAPACITY) * 0.7 / setting.MAX_CAPACITY) < flow['demand']:
				break
			else:
				min_bw = self.get_min_bw_of_links(graph, path, setting.MAX_CAPACITY)
				max_bw = self.get_max_bw_of_links(graph, path, 0)
				x = 1 / ((min_bw * 0.7 / setting.MAX_CAPACITY - flow['demand']) * 0.2)
				y = 1 / ((max_bw - min_bw) * 0.8 / setting.MAX_CAPACITY)
				p = x + y
				global totolp
				totolp += p
		return totolp
		...
		for path in paths:
		    ...
		    min_bw = self.get_min_bw_of_links(graph, path, 10000)
			max_bw = self.get_max_bw_of_links(graph, path, 0)
			x1 = (min_bw / 12500 - flow['demand']) * 0.6
			if x:
				x = 1 / x1
			    y = 1 / ((max_bw - min_bw) * 0.4 / 10000)
				p = x + y
				totolp += p
			else:
				break		
	...
	def _GlobalFirstFit(self, flow):
		...
		paths = self.awareness.shortest_paths.get(src_dp).get(dst_dp)
		GFF_route = None
		totolp = self._GlobalFirstFit1(flow)
		ftotolp = 0.0
		fslice = random.random()
		for path in paths:	
			if (self.get_min_bw_of_links(graph, path, setting.MAX_CAPACITY) * 0.7 / setting.MAX_CAPACITY) < flow['demand']:
	...
	def get_max_bw_of_links(self, graph, path, max_bw):
		"""
			Getting bandwidth of path. Actually, the mininum bandwidth
			of links is the path's bandwith, because it is the bottleneck of path.
		"""
		_len = len(path)
		if _len > 1:
			maxmal_band_width = max_bw
			for i in xrange(_len-1):
				pre, curr = path[i], path[i+1]
				if 'bandwidth' in graph[pre][curr]:
					bw = graph[pre][curr]['bandwidth']
					maxmal_band_width = max(bw, maxmal_band_width)
				else:
					continue
			return maxmal_band_width
		else:
			return max_bw
